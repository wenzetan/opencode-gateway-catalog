...
async function readBodyWithLimit(response, maxBytes, urlLabel) {
    const contentLength = response.headers.get("content-length");
    if (contentLength !== null) {
        const declared = Number(contentLength);
        if (Number.isFinite(declared) && declared > maxBytes) {
            throw new DiscoveryError(`response from ${urlLabel} is too large (${declared} bytes > ${maxBytes} bytes)`);
        }
    }
    ...
    try {
        for (;;) {
            ...
            if (received > maxBytes) {
                throw new DiscoveryError(`response from ${urlLabel} exceeded the ${maxBytes} byte limit while streaming`);
            }
            chunks.push(value);
        }
    }
...
export async function fetchJson(options) {
    ...
    if (response.status === 401 || response.status === 403) {
        ...
        throw new AuthenticationError(`gateway rejected the request with HTTP ${response.status} (check your API key)`);
    }
    if (!response.ok) {
        await response.body?.cancel().catch(() => undefined);
