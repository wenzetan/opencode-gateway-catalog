 ...
 */
import { readCatalogData, readModelItems, isRecord, readBoolean, readNonEmptyString, readNonNegativeNumber, readPositiveInt, readStringArray, } from "../validation.js";
import { fetchJson } from "../discovery.js";
function invalid(state, field, detail) {
    state.issues.push({ id: state.id, kind: "invalid", field, detail });
}
function readField(state, rawValue, field, parser) {
...
function parseCapabilities(state, raw) {
    if (raw === undefined)
        return {};
    if (!isRecord(raw)) {
    ...
    return {
        ...
        thinking: readField(state, raw["thinking"], "capabilities.thinking", readBoolean) ??
            readField(state, raw["supportsThinking"], "capabilities.supportsThinking", readBoolean),
        effortTiers: readField(state, raw["effort_tiers"], "capabilities.effort_tiers", readStringArray),
    };
}
function parsePricing(state, raw) {
    if (raw === undefined)
        return undefined;
    if (!isRecord(raw)) {
    ...
    const pricing = {
        ...
        cacheCreation: readField(state, raw["cache_creation"], "pricing.cache_creation", readNonNegativeNumber),
        reasoning: readField(state, raw["reasoning"], "pricing.reasoning", readNonNegativeNumber),
    };
    return pricing;
}
function parseModel(item, id, issues) {
    const state = { id, issues };
    ...
    return {
        ...
        maxOutputTokens: readField(state, item["max_output_tokens"], "max_output_tokens", readPositiveInt),
        toolCalling: capabilities.toolCalling,
        inputModalities: readField(state, item["input_modalities"], "input_modalities", readStringArray),
        outputModalities: readField(state, item["output_modalities"], "output_modalities", readStringArray),
        vision: capabilities.vision,
        reasoning: capabilities.reasoning,
        thinking: capabilities.thinking,
        effortTiers: capabilities.effortTiers,
        pricing: parsePricing(state, item["pricing"]),
        supportedEndpoints: readField(state, item["supported_endpoints"], "supported_endpoints", readStringArray),
