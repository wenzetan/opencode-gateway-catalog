...
import type { Model, Plugin as PluginApi, Provider } from "@opencode/plugin";
import { createOmniRouteAdapter } from "./adapters/omniroute.js";
import type { GatewayAdapter } from "./adapters/types.js";
import { DiskCache } from "./cache.js";
import { parseConfig, type ResolvedConfig } from "./config.js";
import { describeError } from "./errors.js";
import { createLogger } from "./logger.js";
import { CatalogController, type CatalogState } from "./refresh.js";
...
type ProviderRecord =
  ReturnType<ProviderEditor["get"]> extends infer T
    ? T extends undefined
      ? never
      : NonNullable<T>
    : never;

export function createAdapter(config: ResolvedConfig): GatewayAdapter {
...
export function integrationIdFor(config: ResolvedConfig): string {
  return `gateway-catalog-${config.providerId}`;
}

export function usesEnvIntegration(config: ResolvedConfig): boolean {
  return config.apiKey === undefined && config.apiKeyEnv !== null;
}

export function buildProviderInfo(config: ResolvedConfig): Provider.Info {
  const settings: Record<string, unknown> = { baseURL: config.apiBaseURL };
  ...
  return {
    id: config.providerId as Provider.Info["id"],
    name: config.providerName,
    activation: "enabled",
    package: OPENAI_COMPATIBLE_PACKAGE,
    settings: settings as Provider.Info["settings"],
    ...(usesEnvIntegration(config)
      ? { integrationID: integrationIdFor(config) as Provider.Info["integrationID"] }
      : {}),
  };
}

function isOwnProvider(record: ProviderRecord, config: ResolvedConfig): boolean {
  const provider = record.provider;
  if (provider.package !== OPENAI_COMPATIBLE_PACKAGE) return false;
...
export function registerCatalog(
  ...
  if (existing !== undefined && !isOwnProvider(existing, config)) {
    logger.error(
      `provider "${providerId}" already exists (package=${existing.provider.package ?? "unknown"}); ` +
  ...
  const models: readonly Model.Info[] = state.models;
  logger.debug(
    `transform: provider=${providerId} models=${models.length} existing=${existing === undefined ? "no" : "yes"}`,
  );
...
const plugin: PluginApi.Plugin = {
  ...
  async setup(context) {
    ...
    logger.debug(
      `adapter=${config.adapter} provider=${config.providerId} gatewayRoot=${config.gatewayRoot} apiBaseURL=${config.apiBaseURL} discoveryURL=${config.discoveryURL}`,
    );
    ...
    return () => {
      ...
      controller.dispose();
      logger.debug(
        `provider=${config.providerId} plugin unloaded; timers and in-flight requests cleaned up`,
      );
    };
  },
};
